...
if 'rng' in dir(pyb):
  ...
  import urandom
  def rng():
    return int(urandom.random() * 65536)
...
def box(display):
  ...
  sz = display.size()
  print('size =', sz)
  red = pyb.TFT.color(255, 0, 0)
  display.line((0, 0), sz, red)
  display.line((0, sz[1]), (sz[0], 0), red)

def test():
    # On the G30DEV board, the LCD is connected to SPI2 (which is 'Y' on the pyboard
    # CS is PB1
    # RS is PB0
    # Reset is PB12
    pyb.Pin('LCD_BACKLIGHT', pyb.Pin.OUT_PP).value(1)
    pyb.Pin('LCD_CS', pyb.Pin.OUT_PP).value(1)
    pyb.Pin('LCD_RS', pyb.Pin.OUT_PP).value(1)
    lcd_reset = pyb.Pin('LCD_RESET', pyb.Pin.OUT_PP)
    lcd_reset.value(0)
    pyb.delay(100)
    lcd_reset.value(1)

    display = pyb.TFT('y', 'LCD_RS', 'LCD_RESET')
    display.initg()
    box(display)
